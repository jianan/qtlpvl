...
to have a negative and bigger QTL effect.  The 10 phenotypes are
generated with these QTL effect matrix G plus independent noise
...
```{r}
obj <- testpleio.1vs2(cross=listeria, Y=Y, chr=chr, n.simu=100,
...
```{r}
obj2 <- testpleio.1vsp(cross=listeria, Y=Y, chr=chr, n.simu=100)
summary(obj2)
plot(obj2)
```
...
```{r}
plotGenetpattern(Y=Y, genotype=genotype1)
...
the procedure by skiping mapping the triats redundantly.
