...
genotype1 <- geno[,7]
genotype2 <- geno[,10]
(pos <- unlist(pull.map(listeria, chr="1"))[c(7,10)])
p <- 10
p1 <- floor(p/2)
...
statistic. To get the null distribution of the statistics, we generate
data from the parameters estimated under the null hypothesis, repeat
the whole procedure and save the test statistics. The P-value is
calculated from this empirical distribution.
...
* int.method: can have value of "bayes" or "1.5lod", method to
              calculated the interval of interest if ‘region.l’ and
              ‘region.r’ is not specified.
...
The above figure shows the joint mapping resut in black and profile
...
the cut-point of the left versus right group. There are 7 possile QTL
...
Thus when the first 5 traits and the last 5 triats are grouped
seperately, the LODdiff is the biggest, it is our best two QTL model.
This figure could be used as a diagnositc figure, to see if our best
...
* $H_0$: There is only one QTL affecting all the phenotypes; 
* $H_1$: There are p QTLs, each affecting one of the phenotypes,
         i.e., each phenotype is allowed to has its own QTL.
...
Function `plotGenetpattern` takes two input parameters, a phenotype
matrix `Y` and a genotype vector `genotype`, the later should be
genotype of the common QTL in most suspicious. QTL additive effect is
defined as a=(RR-BB)/2, QTL dominant effect is defined as
d=BR-(BB+RR)/2, when plotted against each other, traits with pure
additive effects are near the x-axis and traits with dominant effects
are along the diagonals, $y=x$ and $y=-x$.

```{r}
plotGenetpattern(Y, genotype1)
```

We can tell from the plot the there are two different genetic
