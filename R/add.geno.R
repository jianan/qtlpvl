...
add.geno <- function(transbands, cross, regn.cM=5, max.p=100){
  ...
  for(i in 1:length(transbands)){  
    ...
    genoprob.pmarker <- genoprob[, grep(pmarker, colnames(genoprob))]
    ...
    geno1 <- apply(genoprob[, grep(pmarker1, colnames(genoprob))], 1, which.max)
    geno2 <- apply(genoprob[, grep(pmarker2, colnames(genoprob))], 1, which.max)
